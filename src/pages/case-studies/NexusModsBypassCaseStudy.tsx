...
/**
 * The gallery removes any capture that fails to load and disappears entirely
 * if none survive. A missing asset should leave no trace, not a broken frame —
 * the captures are generated by `npm run gen:assets` and a checkout that has
 * not run it should still render a clean page.
 */
/**
 * One adaptive frame, one capture, drag or swipe for the next.
 *
 * Only the active capture participates in layout. That lets the frame follow
 * each image's real aspect ratio instead of forcing a portrait popup and a
 * very wide collection screen into the same fixed-height box.
 *
 * A capture that fails to load is dropped, and the whole section disappears if
 * none survive — a missing asset should leave no trace, not an empty frame.
 */
function Gallery({
  label,
  note,
          ...
          >
            <figure className="nxb-slide" key={shot.file} data-shot={shot.file}>
              <img
                ...
                /* A missing file does not always 404: an SPA host answers
                   with index.html and a 200, which loads but decodes to
                   nothing. */
                onLoad={(e) =>
    ...
    <div className="page nxb" data-accent="lime">
      <div className="container nxb__container cs-scope">
        ...
        <header className="nxb-hero">
          ...
          {/* The operation panel. Built from the project's own sketch data and
              labelled as a schematic — it is not pretending to be a capture. */}
